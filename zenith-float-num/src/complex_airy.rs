...
impl ExactComplex {
    ...
    fn ai_at(&self, work_p: usize, dest_p: usize, cc: &mut Consts, connect: bool) -> Self {
        ...
        if connect {
            ...
            let t1 = w.mul(&zw.ai_at(work_p, dest_p, cc, false), work_p, RoundingMode::None);
            let t2 = w2.mul(&zw2.ai_at(work_p, dest_p, cc, false), work_p, RoundingMode::None);
    ...
    fn bi_at(&self, work_p: usize, dest_p: usize, cc: &mut Consts) -> Self {
        ...
        let a2 = self
            ...
            .ai_at(work_p, dest_p, cc, true);
        ep.mul(&a1, work_p, RoundingMode::None)
            .add(&em.mul(&a2, work_p, RoundingMode::None), work_p, RoundingMode::None)
    }

    fn airy_series_pair(&self, p: usize) -> (ExactComplex, ExactComplex) {
        let x3 = self.mul(self, p, RoundingMode::None).mul(self, p, RoundingMode::None);
    ...
    fn airy_series_ai(&self, p: usize, cc: &mut Consts) -> Self {
        ...
        c1c.mul(&f, p, RoundingMode::None)
            .sub(&c2c.mul(&g, p, RoundingMode::None), p, RoundingMode::None)
    }
    ...
    fn airy_series_bi(&self, p: usize, cc: &mut Consts) -> Self {
        ...
        s3.mul(
            &c1c
                ...
                .add(&c2c.mul(&g, p, RoundingMode::None), p, RoundingMode::None),
            p,
            RoundingMode::None,
        )
    }
    ...
    fn airy_xi_z(&self, p: usize, cc: &mut Consts) -> (ExactComplex, ExactComplex) {
        let two_thirds = two_c(p).div(&ExactComplex::from_real(ExactNum::from_u8(3, p), p), p, RoundingMode::None);
    ...
    fn airy_u_sum(&self, xi: &ExactComplex, p: usize, alt: bool) -> ExactComplex {
        ...
        for k in 1..=cap {
            let num = ExactNum::from_u32((6 * k - 5) as u32, p)
                .mul(&ExactNum::from_u32((6 * k - 1) as u32, p), p, RoundingMode::None);
            let den = ExactNum::from_u32(72, p).mul(&ExactNum::from_u32(k as u32, p), p, RoundingMode::None);
    ...
    fn airy_asymp_ai(&self, p: usize, cc: &mut Consts) -> Self {
        ...
        let half = half_c(p);
        let pi = pi_c(p, cc);
        let pi_m12 = ExactComplex::one(p).div(&pi.sqrt(p, RoundingMode::None, cc), p, RoundingMode::None);
...
mod tests {
    ...
    fn test_complex_airy_golds() {
        ...
        assert!(tiny(sum.re(), p) && tiny(sum.im(), p), "connection identity");
