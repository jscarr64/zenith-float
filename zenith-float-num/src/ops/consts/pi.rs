...
fn pqr(a: u64, b: u64) -> Result<(ExactNumNumber, ExactNumNumber, ExactNumNumber), Error> {
    if a == b - 1 {
        let n0 = ExactNumNumber::from_u64(6 * b - 5, 64)?;
        let n1 = ExactNumNumber::from_u64(2 * b - 1, 64)?;
        let n2 = ExactNumNumber::from_u64(6 * b - 1, 64)?;

        let n3 = n0.mul_full_prec(&n1)?;
        let r = n3.mul_full_prec(&n2)?;

        let n0 = ExactNumNumber::from_u64(10939058860032000, 64)?;
        let n1 = ExactNumNumber::from_u64(b, 64)?;
        let n2 = n1.mul_full_prec(&n1)?;
        let n3 = n2.mul_full_prec(&n1)?;
        let q = n0.mul_full_prec(&n3)?;

        let n0 = ExactNumNumber::from_u64(13591409 + 545140134 * b, 64)?;
...
fn pqr_inc(
    ...
    ra: &ExactNumNumber,
    m: u64,
) -> Result<(ExactNumNumber, ExactNumNumber, ExactNumNumber, u64), Error> {
    let b = m * 2;
...
impl PiCache {
    fn calc_pi(p: &ExactNumNumber, q: &ExactNumNumber, k: usize) -> Result<ExactNumNumber, Error> {
        // q*4270934400 / ((p + q*13591409) * sqrt(10005))
        let n0 = ExactNumNumber::from_word(4270934400, 1)?;
        let n1 = ExactNumNumber::from_word(13591409, 1)?;

        let q0 = q.mul_full_prec(&n0)?;
        let q1 = q.mul_full_prec(&n1)?;
        let p0 = p.add_full_prec(&q1)?;

        let f3 = ExactNumNumber::from_word(10005, 1)?;
    ...
    pub(crate) fn for_prec(&mut self, k: usize, rm: RoundingMode) -> Result<ExactNumNumber, Error> {
        ...
        loop {
            ...
            if self.b > kext as u64 {
                ...
                bump_prec_retry(&mut p_wrk, &mut p_inc, k)?;
            }
...
mod tests {
    ...
    fn test_pi_const() {
        ...
        //println!("pi {:?}", c);
        let r = ExactNumNumber::from_raw_parts(
            &[
                2385773790, 1363806329, 991140642, 34324134, 2322058356, 688016904, 2161908945,
