...
fn pqr(a: usize, b: usize) -> Result<(ExactNumNumber, ExactNumNumber, ExactNumNumber), Error> {
    if a == b - 1 {
        let p = ExactNumNumber::from_word(81, 1)?;
        let q = ExactNumNumber::from_usize((2 * b + 1) * 121)?;
        let r = ExactNumNumber::from_usize((2 * b + 1) * 81)?;
...
fn pqr_inc(
    ...
    ra: &ExactNumNumber,
    m: usize,
) -> Result<(ExactNumNumber, ExactNumNumber, ExactNumNumber, usize), Error> {
    let b = m * 2;
...
impl Ln10Cache {
    ...
    fn calc_ln10(p: &ExactNumNumber, q: &ExactNumNumber) -> Result<ExactNumNumber, Error> {
        ...
        let f0 = ExactNumNumber::from_word(18, 1)?;
        let f1 = ExactNumNumber::from_word(11, 1)?;
