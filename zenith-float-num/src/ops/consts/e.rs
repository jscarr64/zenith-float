...
use crate::common::util::{bump_prec_retry, log2_floor, round_p};
use crate::defs::Error;
use crate::num::ExactNumNumber;
use crate::{RoundingMode, WORD_BIT_SIZE};

fn pq(a: usize, b: usize) -> Result<(ExactNumNumber, ExactNumNumber), Error> {
    if a == b - 1 {
        let q = ExactNumNumber::from_usize(b)?;
        let p = ExactNumNumber::from_word(1, 1)?;
...
fn pqr_inc(
    ...
    qa: &ExactNumNumber,
    m: usize,
) -> Result<(ExactNumNumber, ExactNumNumber, usize), Error> {
    let b = m * 2;
...
impl ECache {
    fn calc_e(p: &ExactNumNumber, q: &ExactNumNumber) -> Result<ExactNumNumber, Error> {
        // 1 + pk / qk
