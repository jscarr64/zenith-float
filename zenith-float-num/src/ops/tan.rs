...
impl PolycoeffGen for TanPolycoeffGen {
    fn next(&mut self, _rm: RoundingMode) -> Result<&ExactNumNumber, Error> {
        Ok(&ONE)
    }
...
impl ExactNumNumber {
    ...
    fn tan_series_run(&self, rm: RoundingMode) -> Result<Self, Error> {
        ...
        xx.inv_sign();
        let mut xxacc = ExactNumNumber::from_word(1, 1)?;
        let mut fct = ExactNumNumber::from_word(2, 1)?;
        let mut inc = ExactNumNumber::from_word(2, 1)?;
        let mut q1 = ExactNumNumber::from_word(1, 1)?;
        let mut p1 = ExactNumNumber::from_word(1, 1)?;
        let mut q2 = ExactNumNumber::from_word(1, 1)?;
        let mut p2 = ExactNumNumber::from_word(1, 1)?;
...
mod tests {
    ...
    fn test_tangent() {
        ...
        let mut n1 = ExactNumNumber::from_word(2, p).unwrap();
        n1.set_exponent(0);
        ...
        let n2 = half_pi.tan(p, rm, &mut cc).unwrap();
        let n3 = ExactNumNumber::parse("4.ECDEC5EF3A1EA5339A46BC0C490F52A86A033C56BCDD413E36C657EB7757F073500B013B9A7B43C0_e+0", crate::Radix::Hex, p, RoundingMode::None, &mut cc).unwrap();
