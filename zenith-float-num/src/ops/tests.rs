...
fn test_ln_exp() {
    ...
    /*     let e_const = E.with(|v| -> ExactNumNumber {
        v.borrow_mut().for_prec(320, RoundingMode::None).unwrap()
    });

    let d1 = ExactNumNumber::from_raw_parts(&[2134170684, 3164033087, 409012923, 368468195, 719743879, 1804695412, 4180589568, 1528545767, 3297688378, 3632932384], 320, Sign::Pos, 4).unwrap();
...
fn test_sin_asin() {
    ...
    /*     let d1 = ExactNumNumber::from_raw_parts(&[2097186588, 2125458061, 154726044, 1972526461, 2656367726, 814809964, 990939464, 2788161723, 3328293782, 3887912150], 320, Sign::Neg, 0).unwrap();
...
fn test_err() {
    ...
    let mut n = ExactNumNumber::from_words(&[0, 0, 0, 0, 0, 0, 0, crate::WORD_SIGNIFICANT_BIT], Sign::Pos, 1).unwrap();
    ...
    let mut maxeer = -1000;
    for _ in 0..10000 {
        let mut n = ExactNumNumber::random_normal(128 + 64, -10, 10).unwrap();
        n.set_sign(Sign::Pos);
        let mut n1 = n.clone().unwrap();
        n.m.digits_mut()[0] = 0;
        n.m.digits_mut()[1] |= 0;
        ...
        let mut d = ExactNumNumber::random_normal(128 + 64, -10, 10).unwrap();
        d.set_sign(Sign::Pos);
