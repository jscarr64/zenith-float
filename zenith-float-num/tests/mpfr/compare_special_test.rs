...
fn run_compare_special(run_cnt: usize, p_rng: usize, p_min: usize) {
    ...
    for _ in 0..run_cnt {
        ...
        let nn = [
            ExactNum::new(p1),
            ExactNum::new(p1).neg(),
            ExactNum::from_i8(1, p1),
            ExactNum::from_i8(-1, p1),
            ExactNum::max_value(p1),
            ExactNum::min_value(p1),
            ExactNum::min_positive_normal(p1),
            ExactNum::min_positive_normal(p1).neg(),
        ...
        for n in nn.iter() {
            ...
            let nn1 = [
                ExactNum::new(p2),
                ExactNum::new(p2).neg(),
                ExactNum::from_i8(1, p2),
                ExactNum::from_i8(-1, p2),
                ExactNum::max_value(p2),
                ExactNum::min_value(p2),
                ExactNum::min_positive_normal(p2),
                ExactNum::min_positive_normal(p2).neg(),
