...
fn emit(name: &str, x: &ExactNum) {
    let bytes = x.to_bytes().expect("to_bytes of a computed value");
    println!("{name}\t{}", hex_bytes(&bytes));
}

fn emit_c(name: &str, z: &ExactComplex) {
    let re = z.re().to_bytes().expect("complex re to_bytes");
    let im = z.im().to_bytes().expect("complex im to_bytes");
...
fn main() {
    ...
    let z = ExactComplex::new(one.clone(), one.clone());
