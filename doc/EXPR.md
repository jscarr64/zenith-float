...
| `erf` `erfc` `gamma` `ln_gamma` `digamma` `gammainc` `gammainc_upper` `ei` `si` `ci` `li` `fresnel_s` `fresnel_c` `bessel_j` `bessel_j_nu` `bessel_y` `bessel_i` `bessel_k` `elliptic_k` `elliptic_e` `elliptic_e_inc` `elliptic_f` `elliptic_pi` `elliptic_pi_inc` `legendre_p` `legendre_p_assoc` `hypergeom_2f1` `betainc` `normal_pdf` `normal_cdf` `gamma_pdf` `beta_pdf` `poisson_pmf` `binomial_pmf` `chi_squared_cdf` `student_t_pdf` | specials | Same pipeline; MPFR 1-ULP oracles on bounded domains where MPFR has the function |
